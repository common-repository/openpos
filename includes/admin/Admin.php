...
class Openpos_Admin{
    ...
    function plugin_row_meta($plugin_meta, $plugin_file, $plugin_data){
        ...
        if($plugin == 'openpos')
        {
           $plugin_meta[] = '<a target="_blank" href="'.esc_url('https://codecanyon.net/item/openpos-a-complete-pos-plugins-for-woocomerce/22613341').'">'.__('Buy Premium Version','openpos').'</a>';
        }
        return $plugin_meta;
    }
        ...
        $settings_fields = array(
            ...
            'openpos_shipment' => array(
                array(
                    ...
                    'label'   => __( 'POS Addition Shipping Methods', 'openpos' ),
                    'desc'    => __( 'Shipping methods for POS ', 'openpos' ),
                    'type'    => 'multicheck',
                    'default' => '',
    ...
    function pos_admin_menu() {
        $openpos_type = $this->settings_api->get_option('openpos_type','openpos_pos');
        $page = add_menu_page( __( 'Open POS', 'openpos' ), __( 'POS - Lite', 'openpos' ),'manage_woocommerce','openpos-dasboard',array($this,'dashboard'),plugins_url('openpos/assets/images/pos.png'),58 );
        ...
        $page = add_submenu_page( 'openpos-dasboard', __( 'POS - Staffs', 'openpos' ),  __( 'Cashiers', 'openpos' ) , 'manage_options', 'op-cashiers', array( $this, 'cashier_page' ) );
        add_action( 'admin_print_styles-'. $page, array( &$this, 'admin_enqueue' ) );
    ...
    public function dashboard_data($return = false){
        ...
        foreach($orders as $order)
        {
            ...
            $tmp = array(
                ...
                'view' => '<a target="_blank" href="'.get_edit_post_link($id).'">'.__( 'View', 'openpos' ).'</a>'
            );
            $result['order'][] = $tmp;
        }
        $balance = 0;
